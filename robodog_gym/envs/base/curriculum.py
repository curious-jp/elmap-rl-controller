...
class Curriculum:
    def set_to(self, low, high, value=1.0):
        inds = np.logical_and( # only allow command combinations (grid columns) for which all command values are inside the limits.
            self.grid >= low[:, None],
            self.grid <= high[:, None]
        ).all(axis=0)
        ...
        self.weights[inds] = value # set the weights for these grid columns to 1. They can now be sampled.
    ...
    def __init__(self, seed, **key_ranges):
        ...
        self.lows = np.array([range[0] for range in key_ranges.values()]) # lower command limits
        self.highs = np.array([range[1] for range in key_ranges.values()]) # upper command limits
        ...
        self._raw_grid = np.stack(np.meshgrid(*cfg.values(), indexing='ij')) # for n keys: meshgrid creates n n-D objects. These are asymmetical tensors representing the command space. These tensors are then stacked, making a single n+1-D object. The new n+1-th dimension has exactly n entries and represents the different command types. Indexing the remaining n-D tensors separately will give values for each command type.
        self._idx_grid = np.stack(np.meshgrid(*indices.values(), indexing='ij')) 
        self.keys = [*key_ranges.keys()]
        self.grid = self._raw_grid.reshape([len(self.keys), -1]) # flatten the grid. it is now a n x (l) matrix. The rows represent command types, and the columns represent all combinations of command values.
        self.idx_grid = self._idx_grid.reshape([len(self.keys), -1])
        # self.grid = np.stack([params.flatten() for params in raw_grid])

        self._l = l = len(self.grid[0]) # total number of command combinations (grid columns)
        self.ls = {key: len(self.cfg[key]) for key in self.cfg.keys()} # dictionary containing number of possible command values for each command type

        self.weights = np.zeros(l) # probabilities of sampling a command combination. 1 number for each grid column.
    ...
    def sample_bins(self, batch_size, low=None, high=None):
        ...
        if low is not None and high is not None: # if bounds given
            valid_inds = np.logical_and( # only allow command combinations (grid columns) for which all command values are inside the limits.
                self.grid >= low[:, None],
                self.grid <= high[:, None]
            ).all(axis=0)
            temp_weights = np.zeros_like(self.weights)
            temp_weights[valid_inds] = self.weights[valid_inds] # new weight vector, which has all filtered column weights set to 0.
            inds = self.rng.choice(self.indices, batch_size, p=temp_weights / temp_weights.sum()) # sample
        else: # if no bounds given
            inds = self.rng.choice(self.indices, batch_size, p=self.weights / self.weights.sum()) # sample

        return self.grid.T[inds], inds # return sampled command combinations and their indices

    def sample_uniform_from_cell(self, centroids): # sample INSIDE an already sampled command combination
    ...
    def sample(self, batch_size, low=None, high=None): # sample bins and then sample inside the bins
